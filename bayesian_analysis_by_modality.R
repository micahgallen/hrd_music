...
fit_bayesian_psychometric <- function(df, participant_id = "Unknown", modality = "Unknown",
  ...
  # Set a valid scheme name for bayesplot functions
  plot_scheme <- if (modality == "Intero") "red" else "blue"
  color_scheme_set(plot_scheme)
  
  # Diagnostic plots (will now use the 'red' or 'blue' scheme)
  ...
  x_pred <- seq(min(stan_data$x, na.rm=TRUE), max(stan_data$x, na.rm=TRUE), length.out = 100)
